...
var ItemsOption = function(_Component) {
    ...
    _createClass(ItemsOption, [{
        ...
        value: function(data, callback, errorCallback) {
            var _this2 = this;
            this._resetCache();
            this._getStore().insert(this._prepareData(data)).done(function(data) {
                if (callback) {
                    callback(data)
                }
                _this2._resetCache()
        ...
        value: function(key, data, callback, errorCallback) {
            ...
            this._getStore().update(storeKey, this._prepareData(data)).done(function(data, key) {
                if (callback) {
                    callback(key, data)
                }
            }).fail(function(error) {
        ...
        value: function(cache, items) {
            ...
            if (itemsExpr) {
                items.forEach(function(item) {
                    return _this4._fillCache(cache, itemsExpr(item))
                })
            }
            var containerChildrenExpr = this._getContainerChildrenExpr();
            if (containerChildrenExpr) {
                items.forEach(function(item) {
                    return _this4._fillCache(cache, containerChildrenExpr(item))
                })
            }
        }
    }, {
